---
title: Bundling Maestro et al. 1998 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Plan de Manejo del Área Protegida Sistema Península Valdés](https://peninsulavaldes.org.ar/descargas/)

[Caille G., González R., Gosztonyi A. y N. Ciocco. 1997. Especies capturadas por la flota de pesca costera en Patagonia. Programa de Biólogos Observadores a bordo, 1993-1996. Informe técnico del Plan de Manejo Integrado de la Zona Costera Patagónica. Fundación Patagonia Natural (Puerto Madryn, Argentina) Nº 27.](http://dx.doi.org/10.13140/RG.2.2.23274.57286)
...
site_dir_name <- "peninsula_valdes"
dataset_dir_name <- "Maestro_et_al_1998"
original_pdf <- "14-Anexos.pdf"
short_name <- "peninula-valdes-maestro-1998"
...
page_args <- c(
  "-a 233.708,154.148,710.303,358.403 -p 25",
  "-a 96.008,154.148,239.828,359.933 -p 26"
)
...
processed_csv <- "14-Anexos_tables_parsed_concatenated.csv"
...
combined_dataframe[22,2:3] = c("Odontesthes smitti", 990516)
combined_dataframe[43,2:3] = c("Xystreurys rasile", 1621432)
...
#Península Valdés
ind_shape <- shapes_processed$geom[which(shapes_processed$name == "Península Valdés")]
...
#Title
title <- "Plan de Manejo del Área Protegida Sistema Península Valdés: Fishes Checklist"
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "Guillermo", 
      surName = "Caille"),
    organizationName = "National University of Patagonia San Juan Bosco"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Raúl", 
      surName = "González"),
    organizationName = "CONICET - National University of Comahue"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Atila", 
      surName = "Gosztonyi"),
    organizationName = "Centro Nacional Patagonico"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Néstor", 
      surName = "Ciocco"),
    organizationName = "National Scientific and Technical Research Council"
  )
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Caille G., González R., Gosztonyi A. y N. Ciocco. 1997. Especies capturadas por la flota de pesca costera en Patagonia. Programa de Biólogos Observadores a bordo, 1993-1996. Informe técnico del Plan de Manejo Integrado de la Zona Costera Patagónica. Fundación Patagonia Natural (Puerto Madryn, Argentina) Nº 27.")
    )
  )
)

citationdoi <- "http://dx.doi.org/10.13140/RG.2.2.23274.57286"
```
...
coverage <- eml$coverage(
  geographicCoverage = eml$geographicCoverage(
    geographicDescription = "Península Valdés",
