---
title: Bundling Fricke et al. 2011 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Fricke, Ronald & Kulbicki, Michel & Wantiez, Laurent. (2011). Checklist of the fishes of New Caledonia, and their distribution in the Southwest Pacific Ocean (Pisces). Stuttgarter Beiträge zur Naturkunde A Neue Serie. 4. 341-463.](https://www.zobodat.at/pdf/Stuttgarter-Beitraege-Naturkunde_NS_4_A_0341-0363.pdf)
...
dataset_dir_name <- "Fricke_et_al_2011"
original_pdf <- ""
short_name <- "new-caledonia-fricke-2011"

```

# Parsing PDF table to CSV

The data for this reference is formatted as an image-based table inside a PDF across multiple sheets. First, we use [pdf_to_table](https://github.com/sunray1/pdf_to_table) to OCR and parse out the table to a CSV. We don't have to do this since we already have the data as a csv.
...
processed_csv <- "checklist-new-caledonia_fricke_et_al_2011.csv"
...
combined_dataframe[55,2:3] = c("Dipturus", 105762)
combined_dataframe[56,2:3] = c("Insentiraja", 269502)
combined_dataframe[195,2:3] = c('Bathycongrus geminus', NA)
combined_dataframe[256,2:3] = c('Galaxias neocaledonicus', 1017351)
combined_dataframe[574,2:3] = c('Bleheratherina pierucciae', 834884)
combined_dataframe[717,2:3] = c('Microphis brevidorsalis', 1017447)
combined_dataframe[718,2:3] = c('Microphis cruentus', 1012306)
combined_dataframe[720,2:3] = c('Microphis retzii', 1018478)
combined_dataframe[989,2:3] = c('Micropterus salmoides', 151295)
combined_dataframe[1016,2:3] = c('Cheilodipterus lineatus', 401515)
combined_dataframe[1035,2:3] = c('Ostorhinchus amboinensis', NA)
combined_dataframe[1049,2:3] = c('Ostorhinchus hyalosoma', NA)
combined_dataframe[1052,2:3] = c('Ostorhinchus selas', 712688)
combined_dataframe[1275,2:3] = c('Acanthopagrus novaecaledoniae', NA)
combined_dataframe[1296,2:3] = c('Upeneus guttatus', 1013371)
combined_dataframe[1398,2:3] = c('Sarotherodon occidentalis', 1013705)
combined_dataframe[1561,2:3] = c('Halichoeres annulatus', 308607)
combined_dataframe[1843,2:3] = c('Oxyeleotris heterodon', 1018349)
combined_dataframe[1960,2:3] = c('Lentipes kaaea', 1009800)
combined_dataframe[1994,2:3] = c('Schismatogobius fuligimentus', 1010052)
combined_dataframe[1996,2:3] = c('Sicyopterus sarasini', 1017136)
combined_dataframe[1997,2:3] = c('Sicyopus chloe', 1533412)
combined_dataframe[2000,2:3] = c('Stiphodon atratus', 1010745)
combined_dataframe[2001,2:3] = c('Stiphodon mele', 475035)
combined_dataframe[2002,2:3] = c('Stiphodon rutilaureus', 1010748)
combined_dataframe[2003,2:3] = c('Stiphodon sapphirinus', 1009171)
combined_dataframe[2168,2:3] = c('Trichogaster pectoralis', 1622276)
...
#Title
title <- "Checklist of the fishes of New Caledonia, and their distribution in the Southwest Pacific Ocean (Pisces): Fishes Checklist"
...
abstract <- eml$abstract(
  para = "A checklist of the fishes of New Caledonia is presented. A total of 2328 species in 246 families have been recorded from the region. Eight of these species are not native, but have been introduced. Five fish families are represented only by introduced species. The native fish fauna of New Caledonia therefore consists of 2320 species in 241 families. The largest families are the Gobiidae, Labridae, Pomacentridae, Serranidae, Apogonidae, Blenniidae, Macrouridae, Myctophidae and Muraenidae. The freshwater fish fauna is dominated by the families Gobiidae, Eleotridae, Anguillidae and Mugilidae. A total of 61 species represent new records from New Caledonia."
)
```
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Fricke, Ronald & Kulbicki, Michel & Wantiez, Laurent. (2011). Checklist of the fishes of New Caledonia, and their distribution in the Southwest Pacific Ocean (Pisces). Stuttgarter Beiträge zur Naturkunde A Neue Serie. 4. 341-463.")
    )
  )
)
...
coverage <- eml$coverage(
  geographicCoverage = eml$geographicCoverage(
    geographicDescription = "New Caledonia",
