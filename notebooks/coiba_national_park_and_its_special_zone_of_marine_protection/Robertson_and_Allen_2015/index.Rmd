---
title: Bundling Robertson and Allen 2015 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[D R Robertson and Gerald R Allen. 2015. Shorefishes of the Tropical Eastern Pacific: online information system. Version 2.0 Smithsonian Tropical Research Institute, Balboa, Panamá. Coiba National Park.](https://biogeodb.stri.si.edu/sftep/en/pages)
...
dataset_dir_name <- "Robertson_and_Allen_2015"
original_pdf <- "Taxonomic_Functional Group_All_Species.csv"
short_name <- "coiba-robertson-allen-2015"

```

# Parsing PDF table to CSV

The data for this reference is formatted as an image-based table inside a PDF across multiple sheets. First, we use [pdf_to_table](https://github.com/sunray1/pdf_to_table) to OCR and parse out the table to a CSV. We don't have to do this since we have the CSV.
...
processed_csv <- "Taxonomic_Functional Group_All_Species.csv"

input_data <- read.csv(paste(path_to_project_root, "datasets", site_dir_name, dataset_dir_name, "raw", processed_csv, sep="/"), header = FALSE)
...
combined_dataframe[142, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Pontinus", "species A", 126170)
combined_dataframe[163,2:3] = c("Cephalopholis colonus", NA)
combined_dataframe[333, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Chaenopsis", "species A", 268702)
combined_dataframe[353, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Bollmannia", "species A", 268559)
combined_dataframe[371,2:3] = c("Tigrigobius inornatus", NA)
...
#Title
title <- "Shorefishes of the Tropical Eastern Pacific: online information system: Coiba: Fishes Checklist"
...
abstract <- eml$abstract(
  para = "Mobilized species checklist from the Shorefishes of the Tropical Easter Pacific, specifically the confirmed (species collected within the park boundaries) species list from Coiba National Park"
)
```
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "D. Ross", 
      surName = "Robertson"),
    organizationName = "Smithsonian Tropical Research Institute"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Gerald R.", 
      surName = "Allen"),
    organizationName = "Conservation International"
  )
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "D R Robertson and Gerald R Allen. 2015. Shorefishes of the Tropical Eastern Pacific: online information system. Version 2.0 Smithsonian Tropical Research Institute, Balboa, Panamá.")
    )
  )
)
