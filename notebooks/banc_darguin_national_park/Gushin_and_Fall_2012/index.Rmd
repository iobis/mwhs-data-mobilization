---
title: Bundling Gushin and Fall 2012 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Gushchin, A.V., Fall, K.O.M. Ichthyofauna of littoral of the gulf Arguin, Mauritania. J. Ichthyol. 52, 160–171 (2012).](https://doi.org/10.1134/S0032945212010055)
...
Note* - the table in this PDF was originally rotated 90 degrees, I pulled these out and rotated them into its own PDF.

```{r}

path_to_project_root <- "../../.."
site_dir_name <- "banc_darguin_national_park"
dataset_dir_name <- "Gushin_and_Fall_2012"
original_pdf <- "S0032945212010055-4-6_rotated.pdf"
...
page_args <- c(
  "-a 71.775,113.355,540.045,243.045 -p 1",
  "-a 71.775,113.355,540.045,243.045 -p 2",
  "-a 63.855,113.355,490.545,243.045 -p 3"
)
...
processed_csv <- "S0032945212010055-4-6_rotated_tables_parsed_concatenated.csv"
...
cleaned_data <- input_data %>%
  filter(!str_detect(species, "sp\\.|gen\\."))

#For elements with an abbreviated genus name, look at the precious element and grab the genus
for (i in 2:nrow(cleaned_data)) {
  if (grepl("[A-Za-z][.,]", cleaned_data$species[i])) {
    previous_species <- strsplit(cleaned_data$species[i - 1], " ")[[1]][1]
    cleaned_data$species[i] <- sub("^[A-Za-z][.,]", paste0(previous_species, " "), cleaned_data$species[i])
  }
}
...
combined_dataframe[17,2:3] = c("Citharichthys stampflii", 275695)
combined_dataframe[67,2:3] = c("Smaris macrophthalmus", 321559)
combined_dataframe[70,2:3] = c("Sparisoma rubripinne", 159301)

#I think this species doesn't exist - typo
combined_dataframe[75,2:3] = c("Spicara spicara", NA)

combined_dataframe[81,2:3] = c("Tilapia guineensis", 367733)
...
# Generate taxonID:
occurrence %<>% mutate(occurrenceID = paste("banc-darguin-gushin-and-fall-2012", "occurrence", vdigest (scientificName, algo="md5"), sep=":"))
```
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "A. V.", 
      surName = "Gushchina"),
    organizationName = "P.P. Shirshov Institute of Oceanology of Russian Academy of Sciences"
), eml$creator(
    individualName = eml$individualName(
      givenName = "Khallagi Ould Mokhamad", 
      surName = "Fall"),
    organizationName = "Mauritanian Institute for Oceanographic Research and Fisheries"
  )
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      citation = "Earl C, Provost P (2023) Ichthyofauna of Littoral of the Gulf Arguin, Mauritania: Fishes Checklist. [Version 1]. UNESCO. Occurrence Dataset DOI",
      bibliography = list(
        citation = "Gushchin, A.V., Fall, K.O.M. Ichthyofauna of littoral of the gulf Arguin, Mauritania. J. Ichthyol. 52, 160–171 (2012). https://doi.org/10.1134/S0032945212010055")
    )
  )
)
```
...
methods <- eml$methods(
  methodStep = eml$methodStep(
    description = "See Github R Project and Notebook - https://github.com/iobis/mwhs-data-mobilization/tree/master/notebooks/banc_darguin_national_park/Gushin_and_Fall_2012 - for dataset construction"
  )
)
