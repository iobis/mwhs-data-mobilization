---
title: Bundling Downing and Stobart 2021 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Downing, Nigel & Stobart, Ben. (2021). Preliminary Report on Check Lists of Reef Fishes at Aldabra, December 2021. Aldabra Marine Programme](https://www.aldabra.org/wp-content/uploads/2022/01/Report-on-December-2021-Aldabra-surveys.pdf)
...
dataset_dir_name <- "Downing_and_Stobart_2021"
original_pdf <- "Report-on-December-2021-Aldabra-surveys-230222-1.pdf"
short_name <- "aldabra-downing-stobart-2021"
...
page_args <- c(
"-a 230.314,75.531,774.288,484.814 -p 13",
"-a 194.595,72.555,766.102,487.046 -p 14",
"-a 193.107,75.531,765.358,484.07 -p 15",
"-a 193.851,74.787,766.102,487.79 -p 16",
"-a 193.107,75.531,765.358,484.814 -p 17",
"-a 193.107,74.787,405.934,489.279 -p 18"
)
...
processed_csv <- "Report-on-December-2021-Aldabra-surveys-230222-1_tables_parsed_concatenated.csv"
...
input_data %<>%
  filter(rowSums(input_data == "") < ncol(input_data))%>%
  mutate(species = paste(X1, X2, sep = " "))

input_data$X1 <- NULL
input_data$X2 <- NULL

input_data <- input_data[-c(75, 114, 153, 192),]
...
locality_points_file <- "Downing_and_Stobart_2021_localities.csv"
...
#set uncertainty to ~50 meters for those with just one point
all_wkts <- wkts %>%
  rowwise() %>%
  mutate(centroids = list(calculate_centroid(geometry))) %>%
  unnest(centroids) %>%
  mutate(coordinateUncertaintyInMeters = ifelse(coordinateUncertaintyInMeters == 0, 50, coordinateUncertaintyInMeters))
...
for (i in 1:nrow(combined_dataframe)) {
  for (j in 2:9) {
    if (grepl("X", combined_dataframe[i, j])) {
      # Create a new row in occ_data
      site <- sub("^[^.]+\\.", "", colnames(combined_dataframe)[j])
      
      if (site == "X3") {
        locality <- "Aldabra Atoll: Au Parc"
        fieldNumber <- "Au Parc" 
      } else if (site == "X4") {
        locality <- "Aldabra Atoll: Anse Malabar"
        fieldNumber <- "Anse Malabar" 
      } else if (site == "X5") {
        locality <- "Aldabra Atoll: Anse Var"
        fieldNumber <- "Anse Var" 
      } else if (site == "X6") {
        locality <- "Aldabra Atoll: between Sites 2&3"
        fieldNumber <- "between Sites 2&3" 
      } else if (site == "X7") {
        locality <- "Aldabra Atoll: NW"
        fieldNumber <- "NW" 
      } else if (site == "X8") {
        locality <- "Aldabra Atoll: Polymnie"
        fieldNumber <- "Polymnie" 
      } else if (site == "X9") {
        locality <- "Aldabra Atoll: Houareau"
        fieldNumber <- "Houareau"
      } else if (site == "X10") {
        locality <- "Aldabra Atoll: Site 1"
        fieldNumber <- "Site 1" 
      }
      row_index <- which(all_wkts$Group.1 == fieldNumber)
      new_row <- data.frame(
...
#Title
title <- "Preliminary Report on Check Lists of Reef Fishes at Aldabra, December 2021"
...
abstract <- eml$abstract(
  para = "From 9th to 12th December 2021 a series of dives were carried out on the outer reef of Aldabra along the north and west sides of the atoll. The purpose was to identify non-cryptic reef fish species between the approximate depths of 10m and 20m at a series of locations specified by Seychelles Island Foundation (SIF) researchers based at Aldabra."
)
```
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "Nigel", 
      surName = "Downing"),
    organizationName = "Aldabra Marine Programme"
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Downing, Nigel & Stobart, Ben. (2021). Preliminary Report on Check Lists of Reef Fishes at Aldabra, December 2021. Aldabra Marine Programme")
    )
  )
)

citationdoi <- "https://www.aldabra.org/wp-content/uploads/2022/01/Report-on-December-2021-Aldabra-surveys.pdf"
```
  ...
  temporalCoverage = eml$temporalCoverage(
    rangeOfDates = eml$rangeOfDates(
      beginDate = eml$beginDate(
        calendarDate = "2021-12-09"
      ),
      endDate = eml$endDate(
        calendarDate = "2021-12-12"
      )
    )
   )
)
