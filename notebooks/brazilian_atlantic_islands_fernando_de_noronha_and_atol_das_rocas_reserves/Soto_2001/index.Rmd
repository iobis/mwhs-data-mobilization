---
title: Bundling Soto 2001 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Soto, J. M. R. 2001. Peixes do arquipélago Fernando de Noronha. Mare Magnum, 1(2): 147-169.](https://www.univali.br/institucional/museu-oceanografico-univali/mare-magnum/volume-1-numero-2/Documents/maremagnum14.pdf)
...
dataset_dir_name <- "Soto_2001"
original_pdf <- "maremagnum14.pdf"
short_name <- "noronha-soto-2001"
...
page_args <- c(
  "-a 220.522,313.491,771.641,566.366 -p 3",
  "-a 84.416,37.559,771.641,277.791 -p 4",
  "-a 92.597,283.741,768.666,534.384 -p 4",
  "-a 79.209,62.847,769.409,312.747 -p 5",
  "-a 81.441,314.234,768.666,557.441 -p 5",
  "-a 95.572,36.072,280.022,254.734 -p 6"
)
...
processed_csv <- "maremagnum14_tables_parsed_concatenated.csv"
...
# Remove Classes and Orders and take rows with more than 1 word and remove numbers 
cleaned_data <- input_data %>%              
     mutate(across(everything(), ~ if_else(str_detect(.x, "ORDEM|CLASSE"), "", .x))) %>%
     filter(str_count(x0, "\\S+") > 1) %>%
     mutate(across(everything(), ~ gsub("^\\d+\\.\\s*", "", .x)))
...
combined_dataframe[39,2:3] = c("Colossoma macropomum", 1020122)
combined_dataframe[117, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Sparisoma", "aff. chrysopterum", 126051)
combined_dataframe[118, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Sparisoma", "aff. viride", 126051)
...
#Title
title <- "Peixes do arquipélago Fernando de Noronha: Fishes Checklist"
...
abstract <- eml$abstract(
  para = "A systematic checklist of fishes of the Fernando de Noronha Archipelago, Brazil (03°50'S, 32°25'W) is presented, based on literature and field works. A total of 167 species (25 Elasmobranchs and 142 Teleosts) of 68 families were reported, including 4 endemic, 3 introduced and 25 never previously reported in this archipelago."
)
```
...
creator <- eml$creator(
  individualName = eml$individualName(
    givenName = "Jules M. R.", 
    surName = "Soto"),
  organizationName = "Museu Oceanográfico do Vale do Itajaí"
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Soto, J. M. R. 2001. Peixes do arquipélago Fernando de Noronha. Mare Magnum, 1(2): 147-169.")
    )
  )
)
