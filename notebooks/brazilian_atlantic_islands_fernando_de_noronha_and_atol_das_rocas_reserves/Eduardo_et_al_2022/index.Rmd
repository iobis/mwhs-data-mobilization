---
title: Bundling Eduardo et al. 2022 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Eduardo, Leandro & Bertrand, Arnaud & Lucena Frédou, Flavia & Villarins, Bárbara & Martins, Júlia & Afonso, Gabriel & Pietsch, Theodore & Fredou, Thierry & Di Dario, Fabio & Mincarone, Michael. (2022). Rich and underreported: First integrated assessment of the diversity of mesopelagic fishes in the Southwestern Tropical Atlantic. Frontiers in Marine Science. 9.](https://doi.org/10.3389/fmars.2022.937154)
...
dataset_dir_name <- "Eduardo_et_al_2022"
original_pdf <- "fmars-09-937154.pdf"
short_name <- "brazilian-islands-eduardo-2022"
...
page_args <- c(
"-a 133.59,54.701,729.724,466.264 -p 5",
"-a 102.332,55.446,729.724,467.008 -p 6",
"-a 103.821,53.213,726.002,467.008 -p 7",
"-a 103.821,53.213,730.468,464.031 -p 8",
"-a 103.821,53.213,727.491,464.776 -p 9",
"-a 102.332,51.724,679.116,464.031 -p 10"
...
processed_csv <- "fmars-09-937154_tables_parsed_concatenated.csv"
...
input_data %<>%
  select(c(Species, Locality))

# Remove lines with colons, lines with only one word and lines that start with "("
cleaned_data <- input_data %>%
  filter(str_count(Species, "\\S+") > 1) %>%
  remove_empty(c("rows", "cols"))

cleaned_data$Species <- gsub("^\\d+", "", cleaned_data$Species)
...
combined_dataframe[45,3:4] = c("Astronesthes atlanticus", 217337)
combined_dataframe[60, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Eustomias", "sp. 1", 126208)
combined_dataframe[61, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Eustomias", "sp. 2", 126208)
combined_dataframe[62, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Eustomias", "sp. 3", 126208)
combined_dataframe[63, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Eustomias", "sp. 4", 126208)
combined_dataframe[64, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Eustomias", "sp. 5", 126208)
combined_dataframe[70,3:4] = c("Melanostomias bartonbeani", 127359)
combined_dataframe[107,3:4] = c("Dasyscopelus obtusirostris", 1525495)
...
locality_points_file <- "Eduardo_et_al_2022_localities.csv"
...
# Generate taxonID:
occurrence %<>% mutate(occurrenceID = paste(short_name, "occurrence", vdigest (paste(scientificName, identificationQualifier, locality), algo="md5"), sep=":"))
...
```{r eventDate}
eventDate <- "2015-08-29/2017-05-10"
...
abstract <- eml$abstract(
  para = "Here, we provide the first integrative study of the biodiversity of mesopelagic fishes of the southwestern Tropical Atlantic (SWTA), based on two expeditions in northeastern Brazil in 2015 and 2017. A full list of mesopelagic fishes of the region is provided, including rare species and new records for the Brazilian Exclusive Economic Zone and the indication of potentially new species in groups such as the Stomiiformes and Beryciformes.")
```
...
creator <- list(eml$creator(
    ...
    individualName = eml$individualName(
      givenName = "Flávia", 
      surName = "Lucena-Frédou"),
    organizationName = "Universidade Federal Rural de Pernambuco (UFRPE)"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Bárbara Teixeira", 
      surName = "Villarins"),
    organizationName = "Universidade Federal do Rio de Janeiro (UFRJ)"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Júlia Rodrigues", 
      surName = "Martins"),
    organizationName = "Universidade Federal do Rio de Janeiro (UFRJ)"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Gabriel Vinícius Felix", 
      surName = "Afonso"),
    organizationName = "Universidade de São Paulo (USP)"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Theodore Wells", 
      surName = "Pietsch"),
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Eduardo, Leandro & Bertrand, Arnaud & Lucena Frédou, Flavia & Villarins, Bárbara & Martins, Júlia & Afonso, Gabriel & Pietsch, Theodore & Fredou, Thierry & Di Dario, Fabio & Mincarone, Michael. (2022). Rich and underreported: First integrated assessment of the diversity of mesopelagic fishes in the Southwestern Tropical Atlantic. Frontiers in Marine Science. 9.")
    )
  )
)

citationdoi <- "https://doi.org/10.3389/fmars.2022.937154"
```
  ...
  temporalCoverage = eml$temporalCoverage(
    rangeOfDates = eml$rangeOfDates(
      beginDate = eml$beginDate(
        calendarDate = "2015-08-29"
      ),
      endDate = eml$endDate(
        calendarDate = "2017-05-10"
      )
    )
   )
)
