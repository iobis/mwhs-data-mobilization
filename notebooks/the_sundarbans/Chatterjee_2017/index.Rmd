...
library(EML)                            # To create eml.xml file
...
The **metadataProvider** is the person or organization responsible for providing documentation for the resource.
...
#Who to contact about the dataset - who to put here? Pieter?
...
Here we add the additionalMetadata element, which is required for a gbif eml file and contains information such as the citation of the dataset, the citation of the original resource and the timestamp of the eml file.
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      citation = "Dataset citation here",
      bibliography = list(
...
methods <- eml$methods(
  methodStep = eml$methodStep(
    description = "See Github Notebook - URL -for dataset construction"
  )
)
...
intellectualRights <- eml$intellectualRights(
  para = "These data are released under Creative Commons Zero (CC-0) - https://creativecommons.org/publicdomain/zero/1.0/. Users may copy, modify, distribute and use the work, including for commercial purposes, without restriction."
)
...
We have to edit the eml file to GBIF-specific requirements that cannot be included in the original eml construction. This includes changing the schemaLocation and rearranging the GBIF element, since the construction automatically arranges the children nodes to alphabetical order.
...
eml_content %>% xml_find_all(".//gbif") %>% xml_add_child(nodesdefinite, .where=0)
