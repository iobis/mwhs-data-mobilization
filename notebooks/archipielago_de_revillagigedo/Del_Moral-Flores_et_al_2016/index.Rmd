---
title: Bundling Del Moral-Flores et al. 2016 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Del Morel-Flores, Luis Fernando & Negrete, Jatziry & Guzmán, Ana Fabiola. (2016). Peces Del Archipiélago De Las Islas Revillagigedo: Una Actualización Sistemática Y Biogeográfica BIOCYT. 9. 596-619. ](https://doi.org/10.22201/fesi.20072082.2016.9.75910)
...
dataset_dir_name <- "Del_Moral-Flores_et_al_2016"
original_pdf <- "document.pdf"
short_name <- "revillagigedo-del-moral-flores-2016"
...
page_args <- c(
  "-a 233.708,83.003,703.418,256.658 -p 10",
  "-a 83.003,82.238,705.713,253.598 -p 11",
  "-a 83.003,83.768,706.478,260.483 -p 12",
  "-a 82.238,82.238,711.068,261.248 -p 13",
  "-a 82.238,82.238,707.243,259.718 -p 14",
  "-a 81.473,83.003,703.418,258.953 -p 15",
  "-a 81.473,82.238,704.183,265.838 -p 16",
  "-a 83.768,84.533,408.128,252.068 -p 17"
)
...
processed_csv <- "document_tables_parsed_concatenated.csv"
...
# Remove Classes, Families and Orders
cleaned_data <- input_data %>%              
     mutate(across(everything(), ~ if_else(str_detect(.x, "Clase|Orden|Familia"), "", .x))) %>%
     filter(x0 != "")
...
combined_dataframe[125,2:3] = c("Platybelone argalus pterura", 293713)
combined_dataframe[275, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Xyrichtys", "sp. A", 126025)
combined_dataframe[294, c("CanonicalFull", "identificationQualifier", "WormsIDs")] <- c("Malacoctenus", "aff. mexicanus", 269781)

combined_dataframe <- combined_dataframe[-c(77, 87, 296),]
...
#Title
title <- "Peces Del Archipiélago De Las Islas Revillagigedo: Una Actualización Sistemática Y Biogeográfica: Fishes Checklist"
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      ...
      surName = "Del Moral-Flores"),
    organizationName = "Universidad Nacional Autónoma de México"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Jatziry Marlene", 
      surName = "Gracian-Negrete"),
    organizationName = "Universidad Nacional Autónoma de México"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Ana Fabiola", 
      surName = "Guzmán-Camacho"),
    organizationName = "Instituto Politécnico Nacional"
  )
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Del Morel-Flores, Luis Fernando & Negrete, Jatziry & Guzmán, Ana Fabiola. (2016). Peces Del Archipiélago De Las Islas Revillagigedo: Una Actualización Sistemática Y Biogeográfica BIOCYT. 9. 596-619. ")
    )
  )
)

citationdoi <- "https://doi.org/10.22201/fesi.20072082.2016.9.75910"
```
