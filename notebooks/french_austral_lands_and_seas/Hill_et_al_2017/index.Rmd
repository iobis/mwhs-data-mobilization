---
title: Bundling Hill et al. 2017 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
site_dir_name <- "french_austral_lands_and_seas"
dataset_dir_name <- "Hill_et_al_2017"
original_pdf <- "Hill_2017.pdf"
short_name <- "french-austral-hill-2017"
...
page_args <- c(
  "-a 58.411,215.416,373.908,352.329 -p 10"
)
...
processed_csv <- "Hill_2017_tables_parsed_1.csv"
...
ind_shape <- shapes_processed$geom[which(shapes_processed$name == "French Austral Lands and Seas")]

#Just get Kerguelen Plateau
ind_shape_polygon <- st_cast(ind_shape, "POLYGON")
ind_shape <- int_shape_polygon[1]
...
```{r locality}
locality <- "French Austral Islands: Kerguelen Plateau"
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "Nicole A.", 
      surName = "Hill"),
    organizationName = "University of Tasmania"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Scott D.", 
      surName = "Foster"),
    ...
    individualName = eml$individualName(
      givenName = "Dirk", 
      surName = "Welsford"),
    organizationName = "Southern Ocean Ecosystems Theme, Australian Antarctic Division"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Philippe", 
      surName = "Koubbi"),
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Hill, NA, Foster, SD, Duhamel, G, Welsford, D, Koubbi, P, Johnson, CR. Model-based mapping of assemblages for ecology and conservation management: A case study of demersal fish on the Kerguelen Plateau. Diversity Distrib. 2017; 23: 1216– 1230.")
    )
  )
)

citationdoi <- "https://doi.org/10.1111/ddi.12613"
```
...
coverage <- eml$coverage(
  geographicCoverage = eml$geographicCoverage(
    geographicDescription = "Kerguelen Plateau",
