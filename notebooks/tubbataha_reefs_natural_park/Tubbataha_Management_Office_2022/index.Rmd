---
title: Bundling Tubbataha Management Office 2022 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
Tubbataha Management Office 2022 Species List (Given directly)
...
dataset_dir_name <- "Tubbataha_Management_Office_2022"
original_pdf <- ""
short_name <- "tubbataha-TBO-2022"

```

# Parsing PDF table to CSV

The data for this reference is formatted as an image-based table inside a PDF across multiple sheets. First, we use [pdf_to_table](https://github.com/sunray1/pdf_to_table) to OCR and parse out the table to a CSV. We don't have to do this since we have the raw data in CSV.
...
processed_csv <- "2022_TRNP_Species_list_fish.csv"
...
combined_dataframe[19,2:3] = c("Stegostoma tigrinum", 313100)
combined_dataframe[44,2:3] = c("Peponocephala electra", 137103)
combined_dataframe[775,2:3] = c("Pomacentrus simsiang", 277171)


combined_dataframe <- combined_dataframe[-c(1, 30, 45),]
...
```{r}
#Remove duplicates
combined_dataframe <- distinct(combined_dataframe)
...
```{r locality}
locality <- "Tubbataha Reefs Natural Park"
...
#Title
title <- "Tubbataha Management Office: Fishes/Megavertebrates Checklist"
...
abstract <- eml$abstract(
  para = "Fish and Megavertebrate Species List"
)
```
...
creator <- eml$creator(
  individualName = eml$individualName(
    givenName = "", 
    surName = "Tubbataha Management Office"),
  organizationName = ""
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Tubbataha Management Office (2022)")
    )
  )
)
