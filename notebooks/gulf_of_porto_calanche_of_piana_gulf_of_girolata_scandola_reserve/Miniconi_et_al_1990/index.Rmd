---
title: Bundling Miniconi et al. 1990 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Miniconi, Roger & Francour, Patrice & C.H, Bianconi. (1990). Inventaire de la faune ichtyologique de la réserve naturelle de Scandola (Corse, Méditerranée nord-occidentale). Cybium: international journal of ichthyology. 14. 77-89. ](https://sfi-cybium.fr/fr/inventaire-de-la-faune-ichtyologique-de-la-r%C3%A9serve-naturelle-de-scandola-corse-m%C3%A9diterran%C3%A9e-nord)
...
dataset_dir_name <- "Miniconi_et_al_1990"
original_pdf <- "1990MiniconietalInventaireFauneichtyologiqueScandola.pdf"
...
page_args <- c(
  "-a 189.095,47.912,649.499,235.589 -p 9",
  "-a 73.994,55.85,631.355,257.135 -p 10",
  "-a 68.324,58.685,620.582,253.733 -p 11",
  "-a 94.406,56.417,535.532,246.362 -p 12"
)
...
processed_csv <- "1990MiniconietalInventaireFauneichtyologiqueScandola_tables_parsed_concatenated.csv"
...
input_data %<>%
  ...
  select(-c(unnamed_1, unnamed_2, unnamed_3))

# Remove lines with colons, lines with only one word and lines that start with "("
cleaned_data <- input_data %>%
  filter(str_count(famille_espece, "\\S+") > 1)

#For elements with an abbreviated genus name, look at the precious element and grab the genus
for (i in 2:nrow(cleaned_data)) {
  if (grepl("[A-Za-z][.,]", cleaned_data$famille_espece[i])) {
    previous_species <- strsplit(cleaned_data$famille_espece[i - 1], " ")[[1]][1]
    cleaned_data$famille_espece[i] <- sub("^[A-Za-z][.,]", paste0(previous_species, " "), cleaned_data$famille_espece[i])
  }
}
...
combined_dataframe[2,2:3] = c("Scyliorhinus canicula", 105814)
combined_dataframe[3,2:3] = c("Scyliorhinus stellaris", 105815)
combined_dataframe[44,2:3] = c("Serranus cabrilla", 127041)
combined_dataframe[45,2:3] = c("Serranus hepatus", 127042)
combined_dataframe[46,2:3] = c("Serranus scriba", 127043)
combined_dataframe[77,2:3] = c("Labrus bimaculatus", 126966)
combined_dataframe[83,2:3] = c("Symphodus melanocercus", 273570)
combined_dataframe[84,2:3] = c("Symphodus ocellatus", 127460)
combined_dataframe[85,2:3] = c("Symphodus roissali", 273573)
combined_dataframe[86,2:3] = c("Symphodus rostratus", 273574)
combined_dataframe[87,2:3] = c("Symphodus tinca", 273575)
combined_dataframe[124,2:3] = c("Trigla lyra", 127266)


combined_dataframe <- combined_dataframe[-c(104),]
...
# Generate taxonID:
occurrence %<>% mutate(occurrenceID = paste("scandola-miniconi-1990", "occurrence", vdigest (scientificName, algo="md5"), sep=":"))
```
...
#Title
title <- "Inventaire de la faune ichtyologique de la réserve naturelle de Scandola (Corse, Méditerranée nord-occidentale)"

#Abstract
abstract <- eml$abstract(
  para = "This inventory is a synthesis of publications on the natural reserve of Scandola and of unpublished observations. A list of 125 species (51 families) is given; they represent 23% of the species of Western Mediterranean. The number of mediterranean endemic species (9.7%) is more weak than in Western Mediterranean (12.9%). The present list is compared to an equivalent inventory build up in the national Park of Port-Cros."
)
```
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "Roger", 
      surName = "Miniconi"),
    organizationName = "Réserves naturelles Cerbicale Lavezzi"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Patrice", 
      surName = "Francour"),
    organizationName = "Laboratoire de Biologie marine et d'Ecologie du Benthos,"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Charles-Henri", 
      surName = "Bianconi"),
    organizationName = "Pare naturel régional de Corse"
  )
)
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      citation = "Earl C, Provost P (2023) Inventaire de la faune ichtyologique de la réserve naturelle de Scandola (Corse, Méditerranée nord-occidentale): Fishes Checklist. [Version 1]. UNESCO. Occurrence Dataset DOI",
      bibliography = list(
        citation = "Miniconi, Roger & Francour, Patrice & C.H, Bianconi. (1990). Inventaire de la faune ichtyologique de la réserve naturelle de Scandola (Corse, Méditerranée nord-occidentale). Cybium: international journal of ichthyology. 14. 77-89.")
    )
  )
)
```
...
methods <- eml$methods(
  methodStep = eml$methodStep(
    description = paste("See Github R Project and Notebook - https://github.com/iobis/mwhs-data-mobilization/tree/master/notebooks/", site_dir_name, "/", dataset_dir_name, " - for dataset construction", sep = "")
  )
)
