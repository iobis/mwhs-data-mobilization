---
title: Bundling Dufour et al. 2007 to a DwC Archive
date: "`r Sys.Date()`"
author: Chandra Earl
output: (function(...) {
...
[Chetati Dufour, Florence & Guidetti, Paolo & Francour, Patrice. (2007). Comparaison des inventaires de poissons dans les aires marines protégées de Méditerranée: influence de la surface et de l'ancienneté. Cybium: international journal of ichthyology. 31.](https://sfi-cybium.fr/sites/default/files/pdfs-cybium/05.Dufour%20424.pdf)
...
Something wierd was happening with the whole file and it wouldn't OCR, so extracted just the pages with the tables

```{r}

path_to_project_root <- "../../.."
site_dir_name <- "gulf_of_porto_calanche_of_piana_gulf_of_girolata_scandola_reserve"
dataset_dir_name <- "Dufour_et_al_2007"
original_pdf <- "05.Dufour_424-8-13.pdf"
short_name <- "gulf-of-porto-dufour-2007"
...
page_args <- c(
"-a 137.966,109.703,709.166,281.509 -p 1",
"-a 93.341,111.934,709.166,277.047 -p 2",
"-a 90.366,113.422,709.166,271.097 -p 3",
"-a 91.853,110.447,710.653,259.941 -p 4",
"-a 90.366,108.216,709.166,268.866 -p 5",
"-a 92.597,108.216,163.997,239.859 -p 6"

)

# Define run parameters (see documentation)
run_parameters <- "-s -c -nh -ocr -# 8"
...
processed_csv <- "05.Dufour_424-8-13_tables_parsed_concatenated.csv"
...
combined_dataframe[9,2:3] = c("Balistes carolinensis", 127395)
combined_dataframe[37,2:3] = c("Brama brama", 126783)
combined_dataframe[40,2:3] = c("Callionymus lyra", 126792)
combined_dataframe[78,2:3] = c("Coryphaena hippurus", 126846)
combined_dataframe[99,2:3] = c("Diplecogaster bimaculata", 126513)
combined_dataframe[108,2:3] = c("Corcyrogobius liechtensteini", 126876)
combined_dataframe[112,2:3] = c("Didogobius splechtnai", 273434)
combined_dataframe[196,2:3] = c("Nettastoma melanurum", 126308)
combined_dataframe[222,2:3] = c("Raja radula", 105889)
combined_dataframe[228,2:3] = c("Sciaena umbra", 127010)
combined_dataframe[259,2:3] = c("Serranus atricauda", 127040)
combined_dataframe[293,2:3] = c("Squalus blainvillei", 217370)
combined_dataframe[308,2:3] = c("Torpedo nobiliana", 157868)
combined_dataframe[327,2:3] = c("Trigla lyra", 127266)

combined_dataframe <- combined_dataframe[-c(176, 179),]
...
#Title
title <- "Comparaison des inventaires de poissons dans les aires marines protégées de Méditerranée : influence de la surface et de l’ancienneté: Fishes Checklist"
...
abstract <- eml$abstract(
  para = "Les inventaires réalisés dans 8 réserves marines de Méditerranée ont été comparés statistiquement après avoir été complétés : Port-Cros (France), Lavezzi et Scandola (Corse, France), Monaco, Mèdes (Espagne), Termiti, Torre Guaceto et Porto Cesareo (Italie). Ces aires marines protégées présentent des biotopes comparables, mais des dates de création (1963, Port-Cros à 1997, Porto Cesareo) et des surfaces différentes (418 ha, Mèdes, à 16600 ha à Porto Cesareo). Les inventaires, réalisés avec des méthodes variées (relevés visuels en plongée sous-marine, anesthésiques, empoisonnements, captures par filets ou lignes), ont été comparés à l’aide des indices développés par Clarke etWarwick (diversité taxinomique moyenne et sa variance). Au total, 215 espèces sont reportées pour Port-Cros, 198 pour Scandola, 191 pour les Lavezzi, 136 pour les Mèdes, 108 pour Monaco, 105 pour Torre Guaceto, 75 pour Tremiti et Porto Cesareo, à comparer aux 331 espèces retenues pour le peuplement théorique en Méditerranée (sous-espèces, espèces profondes ou lessepsiennes exclues). Le nombre d’espèces trouvées à Port-Cros, à Scandola ou aux Lavezzi correspond à la diversité attendue en Méditerranée. La variance de la diversité taxinomique des zones convenablement protégées correspond aussi à celle qui est attendue en Méditerranée. La richesse spécifique et la diversité taxinomique sont corrélées à l’ancienneté des aires protégées, mais pas à leur taille. En terme de mise en place de nouvelles aires protégées, cela signifie que le degré de surveillance et la pérennité des zones protégées sont plus importants que la surface de la réserve pour abriter un peuplement de poissons riche en nombre d’espèces."
)
```
...
creator <- list(eml$creator(
    individualName = eml$individualName(
      givenName = "Florence", 
      surName = "Dufour"),
    organizationName = "Université de Nice-Sophia Antipolis"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Paolo", 
      surName = "Guidetti"),
    organizationName = "DiSTeBA (Department of Biological and Environmental Sciences and Technology)"
  ), eml$creator(
    individualName = eml$individualName(
      givenName = "Patrice", 
      surName = "Fancour"),
...
additionalMetadata <- eml$additionalMetadata(
  metadata = list(
    gbif = list(
      ...
      bibliography = list(
        citation = "Chetati Dufour, Florence & Guidetti, Paolo & Francour, Patrice. (2007). Comparaison des inventaires de poissons dans les aires marines protégées de Méditerranée: influence de la surface et de l'ancienneté. Cybium: international journal of ichthyology. 31. ")
    )
  )
)
